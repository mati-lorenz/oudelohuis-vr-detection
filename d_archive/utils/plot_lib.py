...
plt.rcParams.update({'font.size': 6, 
                    ...
                    'font.sans-serif': ['DejaVu Sans', 'Arial', 'Helvetica'],  # DejaVu Sans first: ships with matplotlib, always available, avoids 'findfont: Arial not found'
...
def proj_tensor(X,W,idx_N,idx_T):
    ...
    Z               = np.dot(W[idx_N].T, X[np.ix_(idx_N,idx_T,np.ones(S).astype(bool))].reshape((np.sum(idx_N),K*S))).reshape(K,S)
    return Z
...
def get_clr_area_labeled(area_labeled):
    area_labeled = np.atleast_1d(area_labeled)
