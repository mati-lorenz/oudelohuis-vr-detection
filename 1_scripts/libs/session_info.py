...
from get_data_folder import get_data_folder
from session import Session
from behaviorlib import * # get support functions for beh analysis 
from psth import * # get support functions for psth generation
from behaviorCAT import compute_dprime_c
...
def load_sessions(protocol, session_list, load_behaviordata=False, load_calciumdata=False, load_videodata=False, 
    ...
    for i, ses in enumerate(session_list):
        ses = Session(
            protocol=protocol, animal_id=session_list[i, 0], sessiondate=session_list[i, 1])
...
def compute_session_dprime(ses, filter_engaged=True):
    """
    Trial-level d' (Macmillan & Creelman signal-detection framework, via
    behaviorCAT.compute_dprime_c) from trialdata['signal']/['lickResponse']
    -- hit rate at signal==100 (max stimulus), false-alarm rate at
    signal==0 (catch trials). Used by filter_sessions' min_dprime option
    below, and also handy to call directly (e.g. for behavior-summary
    plots) without re-deriving this logic.

    Returns (dprime, criterion), or (np.nan, np.nan) if trialdata lacks
    the needed columns, or has no max-signal/catch trials to compute from
    (e.g. after engagement filtering removes them all).
    """
    trial = ses.trialdata
    if 'signal' not in trial.columns or 'lickResponse' not in trial.columns:
        return np.nan, np.nan

    if filter_engaged and 'engaged' in trial.columns:
        trial = trial[trial['engaged'] == 1]

    signal = trial['signal'].to_numpy()
    response = trial['lickResponse'].to_numpy()
    if np.sum(signal == 100) == 0 or np.sum(signal == 0) == 0:
        return np.nan, np.nan

    return compute_dprime_c(signal, response)
...
def filter_sessions(protocols,load_behaviordata=False, load_calciumdata=False,
                    ...
                    min_dprime=None,dprime_filter_engaged=True):
    """
    ...
    filter_performing (bool, default: True):
    Description: For DN-protocol sessions only, additionally filters out sessions where the animal
    wasn't performing the detection task well, using get_idx_performing_sessions (utils/behaviorlib.py):
    the psychometric fit's noise range must bracket threshold (noise_zmin<=perf_zmin_thr and
    noise_zmax>=perf_zmax_thr) and the guess rate must be below perf_guess_thr. This is the same
    performance criterion load_neural_performing_sessions used to apply by hand -- now the default here
    so every script that calls filter_sessions(protocols=['DN'], ...) gets it automatically, with no
    per-script changes needed. Has no effect on non-DN protocols. Set to False to disable (e.g. to
    reproduce old results, or for exploratory passes where you want every DN session regardless of
    performance).
    perf_zmin_thr, perf_zmax_thr, perf_guess_thr, perf_filter_engaged:
    Description: thresholds forwarded to get_idx_performing_sessions when filter_performing=True;
    defaults match get_idx_performing_sessions' own defaults.

    min_dprime (float, default: None):
    Description: Filters sessions to include only those with trial-level d' (Macmillan & Creelman
    signal-detection framework; see compute_session_dprime above) >= this value. Unlike
    filter_performing (a psychometric-curve-fit criterion, DN-only, evaluated once across the whole
    candidate list), this is a simple per-session statistic computed directly from
    trialdata['signal']/['lickResponse'] (hit rate at signal==100, false-alarm rate at signal==0), so
    it applies to any protocol with those columns (e.g. DM/DP/DN) -- protocols lacking them (e.g.
    GR/GN/IM) are unaffected, min_dprime simply has no effect there, same convention as
    filter_performing's "no effect on non-DN protocols". If a session HAS the columns but d' can't be
    computed (e.g. no max-signal or catch trials survive engagement filtering), it is excluded rather
    than silently passed, since min_dprime was explicitly requested. The computed d' and criterion are
    stashed on the session as ses.dprime / ses.criterion either way, for reuse downstream (e.g. behavior
    summary plots) without recomputing.
    Example: 1.0
    dprime_filter_engaged (bool, default: True):
    Description: whether compute_session_dprime restricts to engaged trials (trialdata['engaged']==1)
    before computing d'; only used when min_dprime is not None.
    """
    sessions = []
    ...
    for protocol in protocols:
        protocol_sessions = []   # sessions passing all non-performance criteria for THIS protocol,
                                  # full data load deferred until after the performance filter below
                                  # (performance filtering needs only the already-loaded trialdata/
                                  # sessiondata, not behavior/calcium/video traces)
        for animal_id in os.listdir(os.path.join(get_data_folder(), protocol)):
            for sessiondate in os.listdir(os.path.join(get_data_folder(), protocol, animal_id)):
                ...
                # SELECT BASED ON D-PRIME (signal-detection performance) -- see filter_sessions
                # docstring for min_dprime. A simple per-session statistic (unlike filter_performing
                # below, which needs the whole candidate list at once), so it's applied here inline
                # alongside the other per-session trial-based filters.
                if sesflag and min_dprime is not None:
                    ses.dprime, ses.criterion = compute_session_dprime(ses, filter_engaged=dprime_filter_engaged)
                    sesflag = sesflag and not np.isnan(ses.dprime) and ses.dprime >= min_dprime
                ...
                if sesflag: #if session meets all criteria so far, queue it (full data loaded below,
                            #after the performance filter, so performance filtering doesn't pay for
                            #loading behavior/calcium/video data on sessions it's about to drop):
                    protocol_sessions.append(ses)

        # SELECT BASED ON TASK PERFORMANCE (DN protocol only -- see filter_sessions docstring).
        # Done once per protocol, after all its candidate sessions are collected, since
        # get_idx_performing_sessions (utils/behaviorlib.py) fits/compares psychometric curves
        # across the whole session list at once rather than session-by-session.
        if filter_performing and protocol == 'DN' and len(protocol_sessions) > 0:
            idx_perf = get_idx_performing_sessions(
                protocol_sessions, zmin_thr=perf_zmin_thr, zmax_thr=perf_zmax_thr,
                guess_thr=perf_guess_thr, filter_engaged=perf_filter_engaged)
            protocol_sessions = [protocol_sessions[i] for i in np.where(idx_perf)[0]]

        for ses in protocol_sessions:
